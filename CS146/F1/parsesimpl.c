...
struct StmtAST parseStmt(struct SExp sexp) {
  ...
  if (!strcmp(t.lexeme, "skip")) {
    ...
    if (n < 2) {
      fprintf(stderr, "Seq statement takes at least one argument\n");
      result.type = BADSTMT;
      return result;
    }
    ...
    struct StmtAST stmts = parseSequence(lst->rest->rest);
    if (stmts.type == BADSTMT) {
      freeStmt(stmt1);
      result.type = BADSTMT;
      return result;
    }
    result.type = SEQ;
    result.seq = makeSeq(stmt1, stmts);
    return result;
  }
